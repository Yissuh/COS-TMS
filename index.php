<?php
error_reporting(E_ALL);
ini_set('display_errors', 1);
require_once 'db_connect.php';
require_once 'fetch_upload.php';
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="index.css">

    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter+Tight:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">

    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
</head>
<body>

    <!-- Header of webpage -->
    <header>
        <picture class="logo">
            <?php
            $imageData = displayImage('full_logo.png', $conn); // Get the image data
            if ($imageData) {
                // If image data is available, create the base64 encoded image source
                $imageSource = 'data:image/png;base64,' . $imageData;
                // Output the image tag with the source set to the base64 encoded image data
                echo '<img src="' . $imageSource . '" style="width:auto;" />';
            } else {
                // Handle case where image data is not available
                echo 'Image not found';
            }
            ?>
        </picture>

        <section class="search">
            <input type="search" placeholder="Search" id="search_bar" style="padding-left:40px;"/>
        </section>
    </header>

    <!--Side menu shown if screen is large.-->
    <section class="sidemenu">
        <form>
            <h1>Filter by</h1>
            <h2>Year</h2>
                <input type="radio" id="year1" name="year" value="2024">
                <label for="year1">2024</label><br>
                <input type="radio" id="year2" name="year" value="2023">
                <label for="year2">2023</label><br>
                <input type="radio" id="year3" name="year" value="2022">
                <label for="year3">2022</label><br>
                <input type="radio" id="year4" name="year" value="2021">
                <label for="year4">2021</label><br><br>

                <hr class="divide">

            <h2>Program</h2>
                <input type="radio" id="prog1" name="program" value="CS">
                <label for="prog1">Computer Science</label><br>
                <input type="radio" id="prog2" name="program" value="IT">
                <label for="prog2">Information Technology</label><br>
                <input type="radio" id="prog3" name="program" value="IS">
                <label for="prog3">Information System</label><br>
        </form>
    </section>

    <!--Upper menu is shown if screen is small-->
    <section class="uppermenu">
        <h1>Filter by</h1>
        <section class="year">
            <button class="yrButton">Year</button>
            <div class="yrContent">
                <option value="2024">2024</option>
                <option value="2024">2023</option>
                <option value="2024">2022</option>
                <option value="2024">2021</option>
            </div>
        </section>

        <section class="program">
            <button class="progButton">Program</button>
            <div class="progContent">
                <option value="CS">Computer Science</option>
                <option value="IT">Information Technology</option>
                <option value="IS">Information System</option>
            </div>
        </section>
    </section>

    <!--This is where list of papers are shown.-->
    <section class="added">
        <h1>Recently Added</h1>

        <section class="papers" id="scroll">
            <section class="papers_list" id="content">
                
                <section class="papers_1" data-link="view_paper.php">
                    <h2 class="title">Insert Title Here</h2>
                    <h3 class="date">November 27, 2023</h3>
                    <h3 class="paper_program">Computer Science</h4>
    
                    <p class="abstract">Proin facilisis elit risus, id placerat nibh sagittis sed. Nullam aliquam leo quis faucibus dapibus. Cras diam sem, scelerisque nec faucibus non, faucibus quis nibh. Duis turpis nisl, pulvinar eget fringilla quis, ultricies id odio. Ut placerat laoreet dui, vel rutrum purus eleifend vel. In congue blandit nibh ac viverra. Cras vestibulum lacus sit amet viverra efficitur. Nunc mattis, urna ac vestibulum ultrices, nisl sapien sollicitudin metus, in vehicula arcu velit vitae turpis. Aliquam non dui lorem. Maecenas ultrices porta lorem eu vestibulum. Vestibulum vestibulum ut eros condimentum condimentum. Praesent pellentesque aliquet facilisis. Vestibulum accumsan eros a accumsan elementum. Ut nec lobortis augue, tincidunt posuere ligula. Vestibulum massa nibh, lobortis id eros non, pellentesque lobortis turpis.</p>
                </section>
    
                <section class="papers_2" data-link="view_paper.php">
                    <h2 class="title">Ano ba Pwedeng Title</h2>
                    <h3 class="date">October 1, 2023</h3>
                    <h3 class="paper_program">Information Technology</h4>
    
                    <p class="abstract">Proin facilisis elit risus, id placerat nibh sagittis sed. Nullam aliquam leo quis faucibus dapibus. Cras diam sem, scelerisque nec faucibus non, faucibus quis nibh. Duis turpis nisl, pulvinar eget fringilla quis, ultricies id odio. Ut placerat laoreet dui, vel rutrum purus eleifend vel. In congue blandit nibh ac viverra. Cras vestibulum lacus sit amet viverra efficitur. Nunc mattis, urna ac vestibulum ultrices, nisl sapien sollicitudin metus, in vehicula arcu velit vitae turpis. Aliquam non dui lorem. Maecenas ultrices porta lorem eu vestibulum. Vestibulum vestibulum ut eros condimentum condimentum. Praesent pellentesque aliquet facilisis. Vestibulum accumsan eros a accumsan elementum. Ut nec lobortis augue, tincidunt posuere ligula. Vestibulum massa nibh, lobortis id eros non, pellentesque lobortis turpis.</p>
                </section>
    
                <section class="papers_1" data-link="view_paper.php">
                    <h2 class="title">Kahit na anong Gusto Mo</h2>
                    <h3 class="date">September 21, 2023</h3>
                    <h3 class="paper_program">Information System</h4>
    
                    <p class="abstract">Proin facilisis elit risus, id placerat nibh sagittis sed. Nullam aliquam leo quis faucibus dapibus. Cras diam sem, scelerisque nec faucibus non, faucibus quis nibh. Duis turpis nisl, pulvinar eget fringilla quis, ultricies id odio. Ut placerat laoreet dui, vel rutrum purus eleifend vel. In congue blandit nibh ac viverra. Cras vestibulum lacus sit amet viverra efficitur. Nunc mattis, urna ac vestibulum ultrices, nisl sapien sollicitudin metus, in vehicula arcu velit vitae turpis. Aliquam non dui lorem. Maecenas ultrices porta lorem eu vestibulum. Vestibulum vestibulum ut eros condimentum condimentum. Praesent pellentesque aliquet facilisis. Vestibulum accumsan eros a accumsan elementum. Ut nec lobortis augue, tincidunt posuere ligula. Vestibulum massa nibh, lobortis id eros non, pellentesque lobortis turpis.</p>
                </section>
    
                <section class="papers_2" data-link="view_paper.php">
                    <h2 class="title">Nakaka-Rawr</h2>
                    <h3 class="date">September 15, 2023</h3>
                    <h3 class="paper_program">Information Technology</h4>
    
                    <p class="abstract">Proin facilisis elit risus, id placerat nibh sagittis sed. Nullam aliquam leo quis faucibus dapibus. Cras diam sem, scelerisque nec faucibus non, faucibus quis nibh. Duis turpis nisl, pulvinar eget fringilla quis, ultricies id odio. Ut placerat laoreet dui, vel rutrum purus eleifend vel. In congue blandit nibh ac viverra. Cras vestibulum lacus sit amet viverra efficitur. Nunc mattis, urna ac vestibulum ultrices, nisl sapien sollicitudin metus, in vehicula arcu velit vitae turpis. Aliquam non dui lorem. Maecenas ultrices porta lorem eu vestibulum. Vestibulum vestibulum ut eros condimentum condimentum. Praesent pellentesque aliquet facilisis. Vestibulum accumsan eros a accumsan elementum. Ut nec lobortis augue, tincidunt posuere ligula. Vestibulum massa nibh, lobortis id eros non, pellentesque lobortis turpis.</p>
                </section>

            </section>

        </section>

    </section>


    <script>
        $('section.papers_1').on('click', function(e){
            e.preventDefault();
            window.location.href=$(this).data('link');
        })

        $('section.papers_2').on('click', function(e){
            e.preventDefault();
            window.location.href=$(this).data('link');
        })

        function scrollFunction() {
          const element = document.getElementById("scroll");
          element.scrollIntoView();
        }
    </script>



    
</body>
</html>